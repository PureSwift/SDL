...
public final class SDLTexture {
    ...
    internal let internalPointer: UnsafeMutablePointer<SDL_Texture>
    ...
    public init(renderer: SDLRenderer, format: SDLPixelFormat.Format, access: Access, width: Int, height: Int) throws(SDLError) {
        ...
        let internalPointer = SDL_CreateTexture(renderer.internalPointer,
                                                      SDL_PixelFormat(rawValue: format.rawValue),
                                                      access.internalValue,
                                                      Int32(width),
                                                      Int32(height))
    ...
    public init(unsafePointer pointer: UnsafeMutablePointer<SDL_Texture>) {
    ...
    public func attributes() throws(SDLError) -> Attributes {
        ...
        let format = internalPointer.pointee.format
        let width = internalPointer.pointee.w
        let height = internalPointer.pointee.h

        let properties = SDL_GetTextureProperties(internalPointer)
        let access = Int32(SDL_GetNumberProperty(properties, SDL_PROP_TEXTURE_ACCESS_NUMBER, 0))

        return Attributes(format: SDLPixelFormat.Format(rawValue: format.rawValue),
                          access: SDLTexture.Access(rawValue: access) ?? .static,
                          width: Int(width),
                          height: Int(height))
    }
...
public extension SDLTexture {
    ...
    enum Access: Int32 {
        ...
        internal var internalValue: SDL_TextureAccess {
            SDL_TextureAccess(rawValue: UInt32(rawValue))
        }
    }
}
...
public extension SDLTexture {
    ...
    enum ScaleMode {
        ...
        internal var internalValue: SDL_ScaleMode {
            switch self {
            case .nearest: return SDL_SCALEMODE_NEAREST
            case .linear: return SDL_SCALEMODE_LINEAR
            case .pixelArt: return SDL_SCALEMODE_PIXELART
            }
        }
    }
}
