...
public extension SDLColor {
    ...
    init(format: SDLPixelFormat,
        ...
        self.rawValue = SDL_MapRGBA(format.internalPointer, nil, red, green, blue, alpha)
    }
    ...
    func components(for format: SDLPixelFormat) -> (red: UInt8, green: UInt8, blue: UInt8, alpha: UInt8) {
        ...
        SDL_GetRGBA(rawValue, format.internalPointer, nil, &components.red, &components.green, &components.blue, &components.alpha)
        return components
    }
}
